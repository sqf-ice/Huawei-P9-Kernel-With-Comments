...
void generic_make_request(struct bio *bio)
{
	 ...
	 */
	if (current->bio_list) {// 当前是否还有BIO正在处理，如果正在处理就退出
	...
	do {
		struct request_queue *q = bdev_get_queue(bio->bi_bdev); // 获得device的请求队列的
		bio_latency_check(bio, BIO_PROC_STAGE_GENERIC_MAKE_REQ);
		q->make_request_fn(q, bio); // 参照blk_queue_make_request
		bio = bio_list_pop(current->bio_list); // 处理就提取出来
	} while (bio);
	current->bio_list = NULL; /* deactivate 处理完就清理队列 */
}
