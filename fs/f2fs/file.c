 ...
 * start:一般是0
 * end:一般是INT的最大值
 * dataasync:一般是0,表示node和data同时一起写入
 * */
int f2fs_sync_file(struct file *file, loff_t start, loff_t end, int datasync)
{
	 ...
	 * FI_ISIZE_CHANGED 一般在分配一个新的data block的时候，就会设置
	 * */
...
static ssize_t f2fs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	inode_lock(inode);
	ret = generic_write_checks(iocb, from); // 返回写入数据的长度
	if (ret > 0) {
	...
	if (ret > 0) { // 如果__generic_file_write_iter失败了执行这个函数
		ssize_t err;

		err = generic_write_sync(file, iocb->ki_pos - ret, ret); // 写完之后执行同步操作，判断需不需要O_SYNC类似的操作
		if (err < 0)
			ret = err;
	}
	return ret;
