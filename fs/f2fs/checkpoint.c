...
/*
 * We guarantee no failure on the returned page.
 * 这里的index指的是该页在磁盘上的块地址(f2fs page = block)
 * 同时，对于新创建的页，也使用f2fs所定义的块号，作为radix树的索引
 */
							...
							bool is_meta)
{
	...
	if (unlikely(!is_meta))
		fio.rw &= ~REQ_META;
repeat:
	page = grab_cache_page(mapping, index); // 查找不到就创建
	if (!page) {
		cond_resched();
		goto repeat;
	}
	...
	if (f2fs_submit_page_bio(&fio)) { // 提交bio，进行获取物理信息
...
int get_valid_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	sbi->ckpt = kzalloc(cp_blks * blk_size, GFP_KERNEL); //分配一个checkpoint
	if (!sbi->ckpt)
		return -ENOMEM;
	/*
	 ...
	 */
	cp_start_blk_no = le32_to_cpu(fsb->cp_blkaddr); // 获取cp的起始块号
	cp1 = validate_checkpoint(sbi, cp_start_blk_no, &cp1_version); // 获取第一个checkpoint

	/* The second checkpoint pack should start at the next segment */
	cp_start_blk_no += ((unsigned long long)1) << le32_to_cpu(fsb->log_blocks_per_seg);
	cp2 = validate_checkpoint(sbi, cp_start_blk_no, &cp2_version); // 获取第二个checkpoint

	if (cp1 && cp2) { // 如果 cp1 和 cp2 存在，那么比较cp1和cp2哪个版本比较新
	...
	cp_block = (struct f2fs_checkpoint *)page_address(cur_page); // 获取当前checkpoint所在的block号
	...
	for (i = 1; i < cp_blks; i++) { // 遍历checkpoint
		void *sit_bitmap_ptr;
		unsigned char *ckpt = (unsigned char *)sbi->ckpt;
		cur_page = get_meta_page(sbi, cp_blk_no + i); //获取这个ino所在的page
		sit_bitmap_ptr = page_address(cur_page); // 同时这个checkpoint 也是 sit_bitmap的起始地址
...
retry_flush_dents:
	...
	if (get_pages(sbi, F2FS_DIRTY_DENTS)) { // 获取dirty dentry的数目
		f2fs_unlock_all(sbi);
...
int write_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	err = block_operations(sbi); // 冻结文件系统操作
	if (err)
		goto out;
	...
	unblock_operations(sbi); // 解冻文件系统操作
