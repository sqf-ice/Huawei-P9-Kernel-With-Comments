 ...
 * 根据vfs superblock初始化f2fs_superblock ，结果保存在raw_super，结果没问题的化应该raw_super应该包含了2个f2fs_superblock
 * valid_super_block：表示成功分配的f2fs_superblock的个数
 * recovery: 是否需要recovery
 */
			...
			int *valid_super_block, int *recovery)
{
	...
	super = kzalloc(sizeof(struct f2fs_super_block), GFP_KERNEL); // 初始化vfs super block
	if (!super)
		return -ENOMEM;

	/*
	 * f2fs有两个f2fs_superblock用于备份
	 * */
	for (block = 0; block < 2; block++) {
		...
		if (!*raw_super) {
			...
			*raw_super = super; // 分配 f2fs_super_block
		}
		brelse(bh);
	}
...
try_onemore:
	...
	sbi = kzalloc(sizeof(struct f2fs_sb_info), GFP_KERNEL); //初始化f2fs_sb_info，这个是核心数据结构
	if (!sbi)
		return -ENOMEM;
	...
	sb->s_fs_info = sbi; // 设置私有指针
	default_options(sbi); //初始化一些数据
	/* parse mount options */
	 ...
	 * 初始化meat_inode，这个inode保存了sit、nat、ssa、checkpoint、f2fs_super_block这些的元数据
	 * */
	...
	err = get_valid_checkpoint(sbi); // 赋予sbi里面的checkpoint
	if (err) {
