...
static struct inode *f2fs_new_inode(struct inode *dir, umode_t mode)
{
	...
	inode = new_inode(dir->i_sb); // 走vfs的方式创建一个inode
	if (!inode)
		return ERR_PTR(-ENOMEM);

	f2fs_lock_op(sbi);
	if (!alloc_nid(sbi, &ino)) { // 分配一个nid
		f2fs_unlock_op(sbi);
		err = -ENOSPC;
		goto fail;
	}
	...
	inode_init_owner(inode, dir, mode); // 走vfs的初始化方式

	inode->i_ino = ino; // 将nid作为inode的ino
	inode->i_blocks = 0;
	...
	f2fs_init_extent_tree(inode, NULL); // 初始化extent tree
...
static int f2fs_create(struct inode *dir, struct dentry *dentry, umode_t mode, bool excl)
{
	...
	inode = f2fs_new_inode(dir, mode); // 创建一个新的inode
	if (IS_ERR(inode))
	...
	alloc_nid_done(sbi, ino); // 完成分配nid

	d_instantiate(dentry, inode);
	unlock_new_inode(inode);

	if (IS_DIRSYNC(dir))
		f2fs_sync_fs(sbi->sb, 1); // 如果包含sync标志，那就马上SYNC
	return 0;
out:
